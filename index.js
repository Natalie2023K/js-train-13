...
function manipulateUrl(url) {
  let man = new URL(url);
  man.protocol = 'https';
  man.host = 'newhost.com';
  man.searchParams.append('newParam', 'newValue');
if(man.searchParams.has('oldParam')) {
  man.searchParams.delete('oldParam')
} return man;
...
function searchParamsURL(url) {
  ...
  let URLSearchParams = result.searchParams;
  let dictionary = new Map();
  URLSearchParams.forEach((key, value) => dictionary.set(key, value));
  return dictionary
...
function manipulateSearchParams(paramsObj, newUrl) {
  let result = new URL(newUrl);
  let obj = Object.keys(paramsObj);
  for(let i of obj) {
    result.searchParams.set(i, paramsObj)
  }
...
function createURLWithParams(params, url) {
  ...
  for (const i in params) {
    result.searchParams.set(i, params[i])
  } return result.toString()
...
function appendSearchParam(url, key, value) {
  let result = new URL(url);
  result.searchParams.append(value, key);
...
for(const i of params) {
  result[i] = check.searchParams.has(i)
} return result
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function processUrl(url, options) {
  ...
  if('searchParams' in options) {
    for(let i in options.searchParams) {
      pro.searchParams.append(i, options[i])
    }
  }
  if('protocol' in options) {
    pro.protocol = options.protocol;
  }
  if('host' in options) {
    pro.host = options.host;
  }
  return pro.toString()
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function manipulateQuery(url, options) {
  let mani = new URL(url);
  if(options.has('append')) {
    for( const [key, value] of options.get('append')) {
mani.searchParams.append(key, value)
    }
  }
  if(options.has('delete')) {
    for(const i of options.get('delete') ) {
      mani.searchParams.delete(i)
    }
  }
...
function sortUrlParams(url) {
  let sor = new URL(url);
  let search = Array.from(sor.searchParams.entries());
  search.sort();
  sor.search = '';
  search.forEach(([key, value]) => {
    sor.searchParams.append(key, value)
  })
  return sor.toString()
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
...
function getURLValues(url) {
  ...
  let key = Array.from(URLSearchParams.keys());
  let arr = [];
  for(const i of key) {
   arr[arr.length]= URLSearchParams.getAll(i)
  }
return arr
...
function getUrlKeys(url) {
  let key = new URL(url);
 let result = Array.from (key.searchParams.keys());
 return result;
  // Створюємо новий об'єкт URL з вхідної URL-адреси.
